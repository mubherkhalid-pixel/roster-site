...
(function(){{
  ...
  // دالة محسّنة لفتح Date Picker - تعمل على iOS
  function openPicker(e){{
    ...
    // طرق متعددة لفتح التقويم - تعمل على كل المتصفحات
    setTimeout(function(){{
      try{{
        // الطريقة 1: showPicker (Chrome, Safari الحديث)
      ...
      try{{
        // الطريقة 2: focus + click (iOS Safari)
        picker.focus();
        setTimeout(function(){{
          picker.click();
        }}, 50);
  ...
  // ربط الأحداث بطريقة محسّنة لـ iOS
  tag.style.cursor = 'pointer';
  tag.style.webkitTapHighlightColor = 'rgba(0,0,0,0)'; // إزالة تأثير النقر في iOS
  tag.style.userSelect = 'none';
  tag.style.webkitUserSelect = 'none';
  
  // أحداث الماوس واللمس
  tag.addEventListener('click', openPicker, false);
  ...
  // منع التمرير عند النقر على التاريخ (iOS)
