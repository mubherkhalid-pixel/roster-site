...
def _day_token(v: str) -> str:
    """
    يطبع نص اليوم من الخلية حتى لو كانت الخلية عمودية/مخلوطة
    - نشيل كل شيء غير الحروف
    - نرجع SUN/MON/... إذا موجودة
    """
    s = norm(v).upper()
...
def find_header_row(ws):
    """
    Finds the header row that contains Employee/Staff/Name/الموظف ANYWHERE in the row
    (not only column A). This matches your screenshot where Employee is in a merged box.
    """
...
def find_employee_col(ws, start_row: int, max_scan_rows: int = 200):
    ...
    Works even if Employee header is merged in the middle.
    """
    scores = {}
...
def find_days_row(ws, scan_down: int = 200):
    """
    Finds the row that contains day headers SUN..SAT in the sheet top area.
    Uses _day_token() so it works with rotated / formatted cells.
    """
...
def find_day_col(ws, today_dow: int, today_dom: int):
    """
    Determines today's column based on:
    - A days row (SUN..SAT)
    - The row below it contains day-of-month numbers (1..31)
    Prefers the column that matches BOTH: day-of-week token and day number.
    """
    days_row_idx, days_row_vals = find_days_row(ws, scan_down=220)
    if not days_row_idx:
        return None
    ...
    # 1) Strict: same col has the correct day token AND the date number matches
    candidate_cols = []
    for c, val in enumerate(days_row_vals, start=1):
        if _day_token(val) == token:
    ...
    # 2) Fallback: within day-labeled area, find the column where date number matches
    day_cols = []
    for c, val in enumerate(days_row_vals, start=1):
        if _day_token(val):
            day_cols.append(c)
